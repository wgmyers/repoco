...
const GIT_MAIN = process.env.GIT_MAIN;
...
// get_changes
async function get_changes(allowed_sites) {
  let data;
  const result = [];
...
async function do_publish(site, target, name, email) {
  let data;

  const git = simpleGit(`${GIT_ROOT}/${site}`);
  const commit_opts = {
    "--author": `"${name} <${email}>"`,
    "--allow-empty": null // Needed to ensure post-receive hook triggers
  }

  console.dir(commit_opts);

  try {
    const changes = await get_unpushed_files(site);
    const commit = await git.commit("Updated via Repoco", changes, commit_opts);
    const response = await git.push("origin", GIT_MAIN, { "--push-option": target });
...
// do_update
// IE we do a git pull origin main
// QUERY: do we want to specify origin main here or allow this to be set in repo?
async function do_update(site) {
  ...
  const git = simpleGit(`${GIT_ROOT}/${site}`);
  try {
    const response = await git.pull("origin", GIT_MAIN);
